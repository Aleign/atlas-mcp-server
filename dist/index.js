    ...
    try {
        if (serverInstance) {
            if (serverInstance instanceof McpServer) {
                ...
                logger.info("Main MCP server (stdio) instance closed successfully.", shutdownContext);
            }
...
const start = async () => {
    ...
    let validatedMcpLogLevel = "info"; // Default
    ...
    try {
        ...
        if (transportType === "stdio" && potentialServer instanceof McpServer) {
            ...
            serverInstance = potentialServer;
            logger.debug("Stored HTTP server instance. MCP sessions are per-request.", startupContext);
        }
        else if (transportType === "http" && !potentialServer) {
            logger.debug("HTTP transport started. Server instance not returned by initializeAndStartServer. MCP sessions are per-request.", startupContext);
        }
