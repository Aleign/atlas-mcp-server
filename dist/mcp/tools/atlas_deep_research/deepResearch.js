...
export async function deepResearch(input) {
    ...
    logger.info(`Initiating deep research plan creation for project ID: ${input.projectId}, Topic: "${input.researchTopic}"`, reqContext);
    try {
        ...
        if (input.scopeDefinition) {
            rootTextParts.push(`Scope: ${sanitization.sanitizeString(input.scopeDefinition)}`);
        }
        ...
        for (const subTopic of input.subTopics) {
            ...
            const subTopicText = `Research Question: ${sanitization.sanitizeString(subTopic.question)}\n\nInitial Search Queries: ${searchQueriesString || "None provided"}`;
            ...
            const subTopicTags = [
                ...
                ...(subTopic.initialSearchQueries?.map((kw) => `search-query:${sanitization
                    .sanitizeString(kw) // Create tags for each search query
                    .toLowerCase()
                    .replace(/\s+/g, "-")
                    .slice(0, 50)}`) || []),
            ];
            logger.debug(`Attempting to create sub-topic node with ID: ${subTopicNodeId} for question: "${subTopic.question}"`, { ...reqContext, subTopicNodeId, question: subTopic.question });
            ...
            await KnowledgeService.linkKnowledgeToKnowledge(subTopicNodeId, planNodeId, "IS_SUBTOPIC_OF");
            ...
            if (tasksToCreate) {
                ...
                const taskTitle = `Research: ${sanitization.sanitizeString(subTopic.question)}`;
                const taskDescription = `Investigate the research question: "${sanitization.sanitizeString(subTopic.question)}"\n\nInitial Search Queries: ${searchQueriesString || "None provided"}\n\nAssociated Knowledge Node: ${subTopicNodeId}`;
                ...
                logger.info(`Task ${createdTaskId} created for sub-topic ${subTopicNodeId}.`, { ...reqContext, createdTaskId, subTopicNodeId });
                // Link Task to the Sub-Topic Knowledge Node
                await TaskService.linkTaskToKnowledge(createdTaskId, subTopicNodeId, "ADDRESSES");
                logger.debug(`Linked task ${createdTaskId} to knowledge node ${subTopicNodeId} with ADDRESSES relationship.`, { ...reqContext, createdTaskId, subTopicNodeId });
            }
        ...
        logger.error("Error occurred during deep research plan creation", error, { ...reqContext, ...errorContextDetails });
