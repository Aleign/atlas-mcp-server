...
import { AtlasDeepResearchInputSchema, AtlasDeepResearchOutputSchema, AtlasDeepResearchSchemaShape, } from "./types.js";
/**
...
async function handler(params, context) {
    const reqContext = context?.requestContext ??
    ...
    if (!validationResult.success) {
        logger.error("Invalid input for atlas_deep_research", validationResult.error, {
            // Pass error object
            ...reqContext,
            // errors: validationResult.error.errors, // This is now part of the error object
        });
        throw new McpError(BaseErrorCode.VALIDATION_ERROR, "Invalid input parameters for atlas_deep_research", validationResult.error.format());
    }
    ...
    try {
        ...
        logger.error("Error executing atlas_deep_research", error, {
            ...reqContext,
        ...
        const errMessage = `Atlas deep research tool execution failed (Request ID: ${reqContext.requestId ?? "N/A"}): ${error instanceof Error ? error.message : String(error)}`;
...
const examples = [
    ...
    // Example 1: Structured technical research with comprehensive subtasks
    {
        ...
        researchGoal: "Systematically identify and critically evaluate leading PQC algorithms, analyzing their technical strengths/limitations and projected adoption timelines.",
        scopeDefinition: "Focus on NIST PQC finalists and standardized algorithms with practical implementation potential. Exclude purely theoretical approaches without near-term implementation viability.",
        subTopics: [
            {
                question: "What are the fundamental taxonomic categories of post-quantum cryptography (PQC) and their underlying mathematical foundations?",
            ...
            {
                question: "What are the quantifiable performance characteristics and resource requirements (computational overhead, key/signature sizes) for leading PQC algorithms?",
                initialSearchQueries: [
            ...
            {
                question: "What practical implementation challenges and realistic adoption timelines exist for PQC deployment across critical infrastructure?",
                initialSearchQueries: [
 ...
 */
export function registerAtlasDeepResearchTool(server) {
    registerTool(server, "atlas_deep_research", // Tool name
