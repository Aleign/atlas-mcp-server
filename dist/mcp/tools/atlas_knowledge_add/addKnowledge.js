...
import { AtlasKnowledgeAddSchema } from "./types.js";
import { formatKnowledgeAddResponse } from "./responseFormat.js";
export const atlasAddKnowledge = async (input, context) => {
    let validatedInput;
    ...
    try {
        ...
        logger.error("Failed to add knowledge item(s)", error, {
            ...reqContext,
        ...
        if (error instanceof Error && error.message.includes("Project with ID")) {
            ...
            throw new McpError(ProjectErrorCode.PROJECT_NOT_FOUND, `Project not found: ${projectId}`, { projectId });
        }
        // Convert other errors to McpError
        throw new McpError(BaseErrorCode.INTERNAL_ERROR, `Error adding knowledge item(s): ${error instanceof Error ? error.message : "Unknown error"}`);
    }
};
