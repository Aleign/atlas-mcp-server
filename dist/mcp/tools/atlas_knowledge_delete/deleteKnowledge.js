...
import { AtlasKnowledgeDeleteSchema, } from "./types.js";
import { formatKnowledgeDeleteResponse } from "./responseFormat.js";
export const atlasDeleteKnowledge = async (input, context) => {
    let validatedInput;
    ...
    try {
        ...
        if (validatedInput.mode === "bulk") {
            ...
            if (!deleted) {
                ...
                throw new McpError(BaseErrorCode.NOT_FOUND, `Knowledge item with identifier ${id} not found`, { knowledgeId: id });
            }
        ...
        logger.error("Knowledge item removal operation failed", error, {
            ...reqContext,
            inputReceived: validatedInput ?? input,
        });
        // Translate unknown errors to structured McpError format
        throw new McpError(BaseErrorCode.INTERNAL_ERROR, `Failed to remove knowledge item(s): ${error instanceof Error ? error.message : "Unknown error"}`);
    }
};
