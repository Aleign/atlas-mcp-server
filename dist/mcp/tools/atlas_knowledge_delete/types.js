...
const BulkKnowledgeSchema = z
    ...
    knowledgeIds: z
        .array(z.string())
        .min(1)
        .describe("Collection of knowledge identifiers to remove in a single operation"),
    responseFormat: createResponseFormatEnum()
        .optional()
        .default(ResponseFormat.FORMATTED)
        .describe("Desired response format: 'formatted' (default string) or 'json' (raw object)"),
...
export const AtlasKnowledgeDeleteSchemaShape = {
    mode: z
        .enum(["single", "bulk"])
        .describe("Operation mode - 'single' for individual removal, 'bulk' for batch operations"),
    id: z
        .string()
        .optional()
