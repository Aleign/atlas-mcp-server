...
export class KnowledgeListFormatter {
    format(data) {
        ...
        const knowledgeSections = knowledge
            .map((item, index) => {
            const { id, projectId, projectName, domain, tags, text, citations, createdAt, updatedAt, } = item;
 ...
 */
export function formatKnowledgeListResponse(data, isError = false) {
    const formatter = new KnowledgeListFormatter();
    const formattedText = formatter.format(data); // Assuming data is KnowledgeListResponse
