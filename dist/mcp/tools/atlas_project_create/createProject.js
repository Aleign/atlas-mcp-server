...
import { AtlasProjectCreateSchema } from "./types.js";
import { formatProjectCreateResponse } from "./responseFormat.js";
export const atlasCreateProject = async (input, context) => {
    let validatedInput;
    ...
    try {
        ...
        if (validatedInput.mode === "bulk") {
                ...
                try {
                    ...
                    if (projectData.dependencies && projectData.dependencies.length > 0) {
                        for (const dependencyId of projectData.dependencies) {
                            try {
                                await ProjectService.addProjectDependency(createdProject.id, dependencyId, ProjectDependencyType.REQUIRES, // Use enum member
            ...
            if (dependencies && dependencies.length > 0) {
                for (const dependencyId of dependencies) {
                    try {
                        await ProjectService.addProjectDependency(project.id, dependencyId, ProjectDependencyType.REQUIRES, // Use enum member
                        "Dependency created during project creation");
                    }
                    catch (error) {
                        const depErrorContext = requestContextService.createRequestContext({
        ...
        logger.error("Failed to initialize project(s)", error, {
            ...reqContext,
            inputReceived: validatedInput ?? input, // Log validated or raw input
        });
        // Handle duplicate name error specifically
        if (error instanceof Error && error.message.includes("duplicate")) {
            throw new McpError(ProjectErrorCode.DUPLICATE_NAME, `A project with this name already exists`, {
                name: validatedInput?.mode === "single"
                    ? validatedInput?.name
                    : validatedInput?.projects?.[0]?.name,
            });
        }
        // Convert other errors to McpError
        throw new McpError(BaseErrorCode.INTERNAL_ERROR, `Error creating project(s): ${error instanceof Error ? error.message : "Unknown error"}`);
    }
};
