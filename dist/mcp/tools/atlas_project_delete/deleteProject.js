...
import { AtlasProjectDeleteSchema } from "./types.js";
import { formatProjectDeleteResponse } from "./responseFormat.js";
export const atlasDeleteProject = async (input, context) => {
    let validatedInput;
    ...
    try {
        ...
        if (validatedInput.mode === "bulk") {
            ...
            if (!deleted) {
                ...
                throw new McpError(ProjectErrorCode.PROJECT_NOT_FOUND, `Project with identifier ${id} not found`, { projectId: id });
            }
        ...
        logger.error("Project removal operation failed", error, {
            ...reqContext,
            inputReceived: validatedInput ?? input,
        });
        // Translate unknown errors to structured McpError format
        throw new McpError(BaseErrorCode.INTERNAL_ERROR, `Failed to remove project(s): ${error instanceof Error ? error.message : "Unknown error"}`);
    }
};
