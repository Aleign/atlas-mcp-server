...
const BulkProjectSchema = z
    ...
    projectIds: z
        .array(z.string())
        .min(1)
        .describe("Collection of project identifiers to remove in a single operation"),
    responseFormat: createResponseFormatEnum()
        .optional()
        .default(ResponseFormat.FORMATTED)
        .describe("Desired response format: 'formatted' (default string) or 'json' (raw object)"),
})
    .describe("Batch removal of multiple project entities in a single transaction");
// Schema shapes for tool registration
export const AtlasProjectDeleteSchemaShape = {
    mode: z
        .enum(["single", "bulk"])
        .describe("Operation strategy - 'single' for individual removal with detailed feedback, 'bulk' for efficient batch operations with aggregated results"),
    id: z
        .string()
        .optional()
        .describe("Target project identifier for permanent removal including all associated tasks and knowledge (required for mode='single')"),
    projectIds: z
        .array(z.string())
        .optional()
        .describe("Collection of project identifiers to permanently remove in a single atomic transaction (required for mode='bulk')"),
    responseFormat: createResponseFormatEnum()
        .optional()
        .describe("Desired response format: 'formatted' (default string) or 'json' (raw object)"),
};
