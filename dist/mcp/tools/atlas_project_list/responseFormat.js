...
export class ProjectListFormatter {
    ...
    format(data) {
        ...
        const projectsSections = projects
            ...
            if (project.status)
                projectSection += `Status: ${project.status}\n`;
            // Format URLs array
            if (project.urls) {
 ...
 */
export function formatProjectListResponse(data, isError = false) {
    const formatter = new ProjectListFormatter();
    const formattedText = formatter.format(data); // Assuming data is ProjectListResponse
