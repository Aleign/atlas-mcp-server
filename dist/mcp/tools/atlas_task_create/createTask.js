...
import { AtlasTaskCreateSchema } from "./types.js";
import { formatTaskCreateResponse } from "./responseFormat.js";
export const atlasCreateTask = async (input, context) => {
    let validatedInput;
    ...
    try {
        ...
        if (validatedInput.mode === "bulk") {
            ...
            for (let i = 0; i < validatedInput.tasks.length; i++) {
                ...
                try {
                    ...
                    if (!projectExists) {
                        throw new McpError(ProjectErrorCode.PROJECT_NOT_FOUND, `Project with ID ${taskData.projectId} not found`, { projectId: taskData.projectId });
                    }
                    ...
                    if (taskData.dependencies && taskData.dependencies.length > 0) {
                        for (const dependencyId of taskData.dependencies) {
                            try {
                                await TaskService.addTaskDependency(createdTask.id, dependencyId);
                            }
                            catch (error) {
        ...
        logger.error("Failed to initialize task(s)", error, {
            ...reqContext,
            inputReceived: validatedInput ?? input,
        });
        // Handle duplicate name error specifically
        if (error instanceof Error && error.message.includes("duplicate")) {
            throw new McpError(ProjectErrorCode.DUPLICATE_NAME, `A task with this title already exists in the project`, {
                title: validatedInput?.mode === "single"
                    ? validatedInput?.title
                    : validatedInput?.tasks?.[0]?.title,
                projectId: validatedInput?.mode === "single"
                    ? validatedInput?.projectId
                    : validatedInput?.tasks?.[0]?.projectId,
            });
        }
        // Convert other errors to McpError
        throw new McpError(BaseErrorCode.INTERNAL_ERROR, `Error creating task(s): ${error instanceof Error ? error.message : "Unknown error"}`);
    }
};
