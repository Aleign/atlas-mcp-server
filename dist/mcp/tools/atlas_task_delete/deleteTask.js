...
import { AtlasTaskDeleteSchema } from "./types.js";
import { formatTaskDeleteResponse } from "./responseFormat.js";
export const atlasDeleteTask = async (input, context) => {
    let validatedInput;
    ...
    try {
        ...
        if (validatedInput.mode === "bulk") {
            ...
            if (!deleted) {
                ...
                throw new McpError(BaseErrorCode.NOT_FOUND, `Task with identifier ${id} not found`, { taskId: id });
            }
        ...
        logger.error("Task removal operation failed", error, {
            ...reqContext,
            inputReceived: validatedInput ?? input,
        });
        // Translate unknown errors to structured McpError format
        throw new McpError(BaseErrorCode.INTERNAL_ERROR, `Failed to remove task(s): ${error instanceof Error ? error.message : "Unknown error"}`);
    }
};
