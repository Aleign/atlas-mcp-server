...
import { AtlasTaskUpdateSchema } from "./types.js";
import { formatTaskUpdateResponse } from "./responseFormat.js";
export const atlasUpdateTask = async (input, context) => {
    let validatedInput;
    ...
    try {
        ...
        if (validatedInput.mode === "bulk") {
            ...
            for (let i = 0; i < validatedInput.tasks.length; i++) {
                ...
                try {
                    ...
                    if (!taskExists) {
                        throw new McpError(BaseErrorCode.NOT_FOUND, `Task with ID ${taskUpdate.id} not found`);
                    }
                    // Update the task
                    const updatedTask = await TaskService.updateTask(taskUpdate.id, taskUpdate.updates);
            ...
            if (!taskExists) {
                throw new McpError(BaseErrorCode.NOT_FOUND, `Task with ID ${id} not found`);
            }
            // Update the task
        ...
        logger.error("Failed to modify task(s)", error, {
            ...reqContext,
            inputReceived: validatedInput ?? input,
        });
        // Handle not found error specifically
        if (error instanceof Error && error.message.includes("not found")) {
            throw new McpError(BaseErrorCode.NOT_FOUND, `Task not found: ${error.message}`);
        }
        // Convert generic errors to properly formatted McpError
        throw new McpError(BaseErrorCode.INTERNAL_ERROR, `Failed to modify task(s): ${error instanceof Error ? error.message : "Unknown error"}`);
    }
};
