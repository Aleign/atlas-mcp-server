import { SearchService, } from "../../../services/neo4j/index.js";
...
export const atlasUnifiedSearch = async (input, context) => {
    ...
    try {
        ...
        if (!validatedInput.value || validatedInput.value.trim() === "") {
            throw new McpError(BaseErrorCode.VALIDATION_ERROR, "Search value cannot be empty", { param: "value" });
        }
        let searchResults;
        ...
        if (propertyForSearch) {
            ...
            else if (entityTypesForSearch.includes("project") &&
                (lowerProp === "name" || lowerProp === "description")) {
                shouldUseFullText = true;
            }
            else if (entityTypesForSearch.includes("task") &&
        ...
        if (shouldUseFullText) {
            ...
            logger.info(`Using regex-based search for specific property: '${propertyForSearch}'. Effective fuzzy for SearchService.search (true means contains): ${finalFuzzyForRegexPath}`, {
                ...
                property: propertyForSearch,
                targetEntityTypes: entityTypesForSearch,
                userInputFuzzy: input?.fuzzy, // Log what user actually passed, if anything
        ...
        if (!searchResults || !Array.isArray(searchResults.data)) {
            ...
            throw new McpError(BaseErrorCode.INTERNAL_ERROR, "Received invalid data structure from search service.");
        }
        ...
        logger.error("Failed to perform unified search", error, {
            ...reqContext,
        ...
        if (error instanceof McpError) {
            ...
            throw new McpError(BaseErrorCode.INTERNAL_ERROR, `Error performing unified search: ${errorMessage}`);
        }
    }
};
