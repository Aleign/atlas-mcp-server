...
function isOriginAllowed(req, res) {
    ...
    if (allowed && origin) {
        if (origin === "null") {
            ...
            logger.debug(`Origin is "null" (and server is localhost-bound). Allowing request without credentials. ACAO: "null", ACAC: "false"`, context);
        }
        else {
            // For any other allowed, non-null origin (i.e., whitelisted), reflect it and allow credentials.
            res.setHeader("Access-Control-Allow-Origin", origin);
            res.setHeader("Access-Control-Allow-Credentials", "true");
            logger.debug(`Origin '${origin}' is whitelisted. Allowing with credentials. ACAO: ${origin}, ACAC: "true"`, context);
        }
