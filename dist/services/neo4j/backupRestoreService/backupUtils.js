 ...
 */
export const secureResolve = (basePath, targetPath) => {
    const resolvedTarget = path.resolve(basePath, targetPath);
    if (resolvedTarget.startsWith(basePath + path.sep) ||
 ...
 */
export const manageBackupRotation = async () => {
    ...
    try {
        ...
        const processedDirs = await Promise.all(dirNames.map(async (name) => {
        ...
        if (backupsToDeleteCount > 0) {
            ...
            for (let i = 0; i < backupsToDeleteCount; i++) {
                ...
                try {
                    ...
                    logger.error(`Failed to delete old backup directory ${dirToDelete}: ${errorMsg}`, rmError, { ...baseContext, dirToDelete });
                }
            }
        }
        else {
        ...
        logger.error(`Error during backup rotation management: ${errorMsg}`, error, baseContext);
    }
};
