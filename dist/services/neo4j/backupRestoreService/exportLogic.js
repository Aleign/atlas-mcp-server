...
import { manageBackupRotation, secureResolve, validatedBackupRoot, } from "./backupUtils.js";
/**
 ...
 */
export const _exportDatabase = async () => {
    ...
    try {
        ...
        const nodeLabels = labelsResult.records.map((record) => record.get("label"));
    ...
    `);
        ...
        logger.error(`Database export failed: ${errorMessage}`, error, baseContext);
        if (backupDir && existsSync(backupDir)) {
            if (!backupDir.startsWith(validatedBackupRoot + require("path").sep)) {
                ...
                try {
                    ...
                    logger.error(`Failed to remove partial backup directory ${backupDir}: ${rmErrorMsg}`, rmError, { ...baseContext, cleanupDir: backupDir });
                }
            }
        }
