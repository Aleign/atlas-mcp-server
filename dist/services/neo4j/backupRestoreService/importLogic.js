 ...
 */
export const _importDatabase = async (backupDirInput) => {
    const backupDir = secureResolve(validatedBackupRoot, path.relative(validatedBackupRoot, path.resolve(backupDirInput)));
    if (!backupDir) {
    ...
    try {
        ...
        if (fullExportPath && existsSync(fullExportPath)) {
            ...
            const fullExport = JSON.parse(fullExportContent);
            ...
            const nodeFiles = filesInBackupDir.filter((file) => file.toLowerCase().endsWith(".json") &&
            ...
            for (const nodeFile of nodeFiles) {
                ...
                const nodesToImport = JSON.parse(fileContent);
        ...
        if (relationships.length > 0) {
            logger.info(`Attempting to import ${relationships.length} relationships...`, { ...baseContext, totalRelationships: relationships.length });
            let importedCount = 0;
            let failedCount = 0;
            const relationshipsByType = {};
            for (const rel of relationships) {
                if (!rel.startNodeId || !rel.endNodeId || !rel.type) {
                    logger.warning(`Skipping relationship due to missing critical data (startNodeId, endNodeId, or type): ${JSON.stringify(rel)}`, { ...baseContext, relationshipData: rel });
                    failedCount++;
                    continue;
                }
            ...
            for (const relType in relationshipsByType) {
                if (Object.prototype.hasOwnProperty.call(relationshipsByType, relType)) {
                    const relsOfType = relationshipsByType[relType];
                    const escapedType = escapeRelationshipType(relType);
                    logger.debug(`Processing ${relsOfType.length} relationships of type ${relType} (escaped: ${escapedType})`, { ...baseContext, relType, count: relsOfType.length });
                    for (let i = 0; i < relsOfType.length; i += batchSize) {
                        const batch = relsOfType.slice(i, i + batchSize);
                        const batchNumber = i / batchSize + 1;
                        logger.debug(`Processing batch ${batchNumber} for type ${relType} (size: ${batch.length})`, { ...baseContext, relType, batchNumber, batchSize: batch.length });
                        const relQuery = `
            ...
            `;
                        try {
                            const result = await session.executeWrite(async (tx) => {
                                logger.debug(`Executing UNWIND transaction for type ${relType}, batch ${batchNumber}`, { ...baseContext, relType, batchNumber });
                                const txResult = await tx.run(relQuery, { rels: batch });
                                logger.debug(`UNWIND transaction executed for type ${relType}, batch ${batchNumber}`, { ...baseContext, relType, batchNumber });
                                return txResult.records[0]?.get("createdCount").toNumber() || 0;
                            });
                            importedCount += result;
                            logger.debug(`Successfully created ${result} relationships of type ${relType} in batch ${batchNumber}`, { ...baseContext, relType, batchNumber, count: result });
                        }
                        catch (error) {
                            const errorMsg = error instanceof Error ? error.message : String(error);
                            logger.error(`Failed to create relationships of type ${relType} in batch ${batchNumber}: ${errorMsg}`, error, {
                                ...baseContext,
                                relType,
                                batchNumber,
        ...
        logger.error(`Database import failed: ${errorMessage}`, error, baseContext);
