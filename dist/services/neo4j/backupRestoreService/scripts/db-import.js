 ...
 */
const isValidBackupDir = (backupDir) => {
    ...
    if (!resolvedBackupDir.startsWith(projectRoot + path.sep) &&
        ...
        logger.error(`Invalid backup directory: Path is outside the project boundary.`, new Error("Path security violation: outside project boundary."), { ...reqContext, pathChecked: resolvedBackupDir });
        return false;
    }
    if (!existsSync(resolvedBackupDir) ||
        !lstatSync(resolvedBackupDir).isDirectory()) {
        logger.error(`Invalid backup directory: Path does not exist or is not a directory.`, new Error("Path validation failed: not a directory or does not exist."), { ...reqContext, pathChecked: resolvedBackupDir });
        return false;
    }
    const expectedFiles = [
        "projects.json",
    ...
    for (const file of expectedFiles) {
        ...
        if (!resolvedFilePath.startsWith(resolvedBackupDir + path.sep) &&
            ...
            logger.warning(`Skipping check for potentially unsafe file path: ${resolvedFilePath} (outside ${resolvedBackupDir})`, { ...reqContext, filePath: resolvedFilePath });
            continue;
        }
        if (existsSync(resolvedFilePath)) {
            ...
            if (file !== "full-export.json" && !existsSync(resolvedFilePath)) {
                logger.warning(`Expected backup file not found: ${resolvedFilePath}. Import might be incomplete if not using full-export.json.`, { ...reqContext, missingFile: resolvedFilePath });
            }
        }
...
const runManualImport = async () => {
    ...
    if (args.length !== 1) {
        logger.error("Usage: npm run db:import <path_to_backup_directory>", new Error("Invalid arguments"), requestContextService.createRequestContext({
    ...
    try {
        ...
        const errorToLog = error instanceof Error ? error : new Error(JSON.stringify(error));
