...
export class KnowledgeService {
    ...
    static async addKnowledge(knowledge) {
      ...
      `;
            ...
            const domainName = createdKnowledgeRecord.get("domainName");
            let actualCitations = [];
    ...
    static async linkKnowledgeToKnowledge(sourceId, targetId, relationshipType) {
      ...
      `;
            ...
            const errorMessage = error instanceof Error ? error.message : String(error);
            logger.error("Error linking knowledge items", error, {
                ...reqContext,
                detail: errorMessage,
            });
            throw error;
    ...
    static async getKnowledgeById(id) {
      ...
      `;
            ...
            const citations = record
                .get("citationSources")
                .filter((c) => c !== null); // Filter nulls if no citations found
            return {
                ...knowledge,
    ...
    static async updateKnowledge(id, updates) {
        ...
        try {
            ...
            if (domainUpdateValue) {
                if (typeof domainUpdateValue !== "string" ||
    ...
    static async getKnowledge(options) {
        ...
        try {
            let conditions = [];
            const params = {}; // Initialize empty params
            ...
            const projectIdMatchFilter = options.projectId && options.projectId !== "*"
                ? "{projectId: $projectId}"
                : "";
    ...
    static async addCitations(knowledgeId, citations) {
      ...
      `;
            ...
            logger.error("Error adding citations", error, {
                ...errorContext,
                detail: errorMessage,
            });
            throw error;
