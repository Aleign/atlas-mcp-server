...
export class ProjectService {
     ...
     */
    static async areAllDependenciesCompleted(projectId) {
        const session = await neo4jDriver.getSession();
        try {
      ...
      `;
            ...
            logger.error("Error checking project dependencies completion status", error, { ...errorContext, detail: errorMessage });
            throw error;
        }
        finally {
            await session.close();
    ...
    static async getProjects(options = {}) {
        ...
        try {
            ...
            const { countQuery, dataQuery, params } = buildListQuery(NodeLabels.Project, returnProperties, {
                // Filters
            ...
            const projects = dataResult.map((record) => {
     ...
     */
    static async addProjectDependency(sourceProjectId, targetProjectId, type, // Use the enum
    description) {
        const session = await neo4jDriver.getSession();
        try {
            ...
            const dependencyExists = await Neo4jUtils.relationshipExists(NodeLabels.Project, "id", sourceProjectId, NodeLabels.Project, "id", targetProjectId, RelationshipTypes.DEPENDS_ON);
            if (dependencyExists) {
      ...
      `;
            ...
            logger.error("Error adding project dependency", error, {
                ...errorContext,
                detail: errorMessage,
            });
            throw error;
     ...
     */
    static async removeProjectDependency(dependencyId) {
        const session = await neo4jDriver.getSession();
        try {
            // Query remains the same
            const query = `
      ...
      `;
            ...
            logger.error("Error removing project dependency", error, {
                ...errorContext,
                detail: errorMessage,
            });
            throw error;
     ...
     */
    static async getProjectDependencies(projectId) {
        const session = await neo4jDriver.getSession();
        try {
            // Logic remains the same
