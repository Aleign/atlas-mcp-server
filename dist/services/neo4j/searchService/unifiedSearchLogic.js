 ...
 * @private
 */
async function _searchSingleLabel(labelInput, cypherSearchValue, originalPropertyName, // Used for Cypher query (case-sensitive)
normalizedLogicProperty, // Used for internal logic (lowercase)
taskTypeFilter, limit = 50, assignedToUserIdFilter) {
    let session = null;
    ...
    try {
        ...
        switch (labelInput.toLowerCase()) {
            ...
            case "knowledge":
                ...
                break;
            default:
                logger.warning(`Unsupported label provided to _searchSingleLabel: ${labelInput}`, reqContext_single);
                return [];
        }
        ...
        const matchClauses = [`MATCH (n:${correctlyEscapedLabel})`];
        let whereConditions = [];
        if (taskTypeFilter) {
        ...
        let propertyForCypher; // This will be original case, or default
        let propertyForLogic; // This will be lowercase, or default
        ...
        if (!propertyForCypher) {
            logger.warning(`Could not determine a search property for Cypher for label ${actualLabel}. Returning empty results.`, { ...reqContext_single, actualLabel });
            return [];
        }
        ...
        if (propertyForLogic === "tags") {
            ...
            else if (tagSearchTerm.startsWith(".*") &&
    ...
    `;
      ...
      LIMIT $limit
    `;
        logger.debug(`Executing search query for label ${actualLabel}. Property for Cypher: '${propertyForCypher}', Property for Logic: '${propertyForLogic}', SearchValue (Regex): '${params.searchValue}'`, {
            ...reqContext_single,
            actualLabel,
            propertyForCypher,
            propertyForLogic,
        ...
        logger.error(`Error searching label ${labelInput}`, error, {
            ...reqContext_single,
            detail: errorMessage,
        });
        return [];
 ...
 */
export async function _searchUnified(options) {
    ...
    try {
        const { property = "", value, entityTypes = ["project", "task", "knowledge"], caseInsensitive = true, fuzzy = false, taskType, assignedToUserId, page = 1, limit = 20, } = options;
        if (!value || value.trim() === "") {
        ...
        for (const label of targetLabels) {
            ...
            searchPromises.push(_searchSingleLabel(
            // Call the local helper
            label, cypherSearchValue, property, normalizedProperty, label.toLowerCase() === "project" || label.toLowerCase() === "task"
                ? taskType
                : undefined, perLabelLimit, label.toLowerCase() === "task" ? assignedToUserId : undefined));
        }
