...
export class Neo4jUtils {
    ...
    static async initializeSchema() {
        ...
        try {
                ...
                for (const query of [...constraints, ...indexes, ...fullTextIndexes]) {
                    try {
                        ...
                        if (query.includes("FULLTEXT")) {
                            ...
                            logger.error(`Failed to execute schema query: ${errorMessage}. Query: ${query}`, error, { ...reqContext, queryFailed: query });
            ...
            const errorMessage = error instanceof Error ? error.message : String(error);
            logger.error("Failed to initialize Neo4j database schema", error, { ...reqContext, detail: errorMessage });
            throw error;
        }
        finally {
            await session.close();
     ...
     */
    static async nodeExists(label, property, value) {
        const session = await neo4jDriver.getSession();
        try {
      ...
      `;
            ...
            logger.error(`Error checking node existence for ${label} {${property}: ${value}}`, error, { ...errorContext, detail: errorMessage });
    ...
    static async relationshipExists(startLabel, startProperty, startValue, endLabel, endProperty, endValue, relationshipType) {
      ...
      `;
            ...
            logger.error(`Error checking relationship existence: (${startLabel})-[:${relationshipType}]->(${endLabel})`, error, { ...errorContext, detail: errorMessage });
            throw error;
        }
        finally {
            await session.close();
